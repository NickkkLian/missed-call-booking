...
5. Send events in the shape shown below. Customer payloads cannot choose their actor: each authenticated route overwrites it and restricts permitted event types. The staff route must be reachable only by the staff tool. Customer-provider adapters must validate provider signatures and bind the phone to the sender; this template does not do that itself. The optional Worker in `worker/` does it for Twilio (see *Live backend* above).
...
## Live backend (optional, not deployed)

`worker/` holds a Cloudflare Worker that connects the live path. It verifies Twilio's `X-Twilio-Signature` and rejects unsigned or forged requests with 403. It turns an inbound call or a WhatsApp sandbox message into a `missed_call` or `message` event in the contract below, and forwards the event to an n8n webhook. `worker/n8n/callback-desk-live.json` is a smaller n8n workflow for that path. It keeps a pending request per phone, and it creates a Google Calendar entry only after a staff decision that matches the request's current revision. Customer-facing texts stay drafts that need staff approval; this milestone sends none. Secrets live only in Worker secrets and n8n credentials, and the export's credential slots are `REPLACE_*` placeholders.
...
docs/backend.js         optional live-backend hook for the page (empty URL = demo mode, the default)
docs/SETUP-BACKEND.md   accounts and steps to deploy the live path
worker/src/index.js     Cloudflare Worker: Twilio signature check, event normalising, forwarding to n8n
worker/wrangler.toml    Worker config, no secrets
worker/n8n/             live n8n export (generated by build-workflow.mjs from steps.cjs)
worker/test/            local tests for the Worker, the live export and the page hook
```
