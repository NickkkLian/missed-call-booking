...
function check(w=read('workflow.json')){
  ...
  const covered=readmeCoverage();assert.ok(covered!==null,'README.md lacks the <!-- coverage:start/end --> markers');
  assert.equal(covered,coverage().trim(),'README coverage table differs from `node demo.js coverage`');
  console.log('PASS README coverage table matches the fixtures');
  console.log('CHECK PASS '+scenarios.length+'/'+scenarios.length+' scenarios');
}
function coverage(){
  // Markdown coverage table generated from the fixtures: which scenarios exercise each event type, route and final state.
  const rows=fixtures();const list=idx=>idx.length?idx.join(', '):'—';
  const byType=t=>rows.map((s,i)=>s.events.some(e=>e.body.type===t)?i+1:null).filter(Boolean);
  const byRoute=r=>rows.map((s,i)=>s.events.some(e=>e.channel===r)?i+1:null).filter(Boolean);
  const byStatus=st=>rows.map((s,i)=>s.expected.status===st?i+1:null).filter(Boolean);
  const out=['| Event type | Scenarios | Count |','| --- | --- | ---: |'];
  for(const t of ['missed_call','message','details','approve_message','approve_booking','calendar_result','tick'])out.push(`| \`${t}\` | ${list(byType(t))} | ${byType(t).length} |`);
  out.push('','| Route | Scenarios | Count |','| --- | --- | ---: |');
  for(const r of ['customer','staff','adapter','timer'])out.push(`| ${r} | ${list(byRoute(r))} | ${byRoute(r).length} |`);
  out.push('','| Final status | Scenarios | Count |','| --- | --- | ---: |');
  for(const st of ['booked','calendar_pending','calendar_failed','awaiting_staff','needs_details','waiting_customer','closed_no_reply','opted_out'])out.push(`| \`${st}\` | ${list(byStatus(st))} | ${byStatus(st).length} |`);
  out.push('',`Scenario numbers follow \`scenarios.json\` order (1–${rows.length}). Generated by \`node demo.js coverage\`; \`check\` fails if this table and the README drift apart.`);
  return out.join('\n');
}
function readmeCoverage(){
  const md=fs.readFileSync(path.join(ROOT,'README.md'),'utf8');const m=/<!-- coverage:start -->\n([\s\S]*?)\n<!-- coverage:end -->/.exec(md);
...
try{
  ...
  else if(command==='coverage')console.log(coverage());
  else throw new Error('Usage: node demo.js build|check|break|simulate|coverage');
